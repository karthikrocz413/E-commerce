...
function displayProducts(filtered = products) {
    let prodcutDiv=document.getElementById("products");
    ...
    filtered.forEach((product)=>{
        ...
        productContainer.innerHTML=` <img class="img1" src="${product.image}" alt="">
                ...
                prodcutDiv.appendChild(productContainer);
    })
} 
function addToCart(id) {
    let selectedProduct=products.find((product)=>product.id===id);
    let exisitingItem=cart.find((item)=>item.id===id);
...
function toggleCart(){
  ...
  if(cart.classList.contains("open")){
    ...
    toggleBtn.innerHTML=" <img src='images/cart.jpg' alt='Cart Icon'>";
  }
}
function showToast(){
